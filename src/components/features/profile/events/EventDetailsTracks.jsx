...
const EventDetailsTracks = ({ tracks }) => {
  ...
  const getGenderBadge = (gender) => {
    ...
    const colorClass = colorMap[uppercaseGender] || "bg-gray-50 text-gray-700 border-gray-200/60";

    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${colorClass} font-somar`}>
                ...
                <div className="flex flex-col gap-1.5 mt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 max-h-[120px] overflow-y-auto custom-scrollbar pr-1">
                    {track.grades.map((grade) => (
                      <span
                        key={grade._id}
                        className="px-2 py-0.5 rounded bg-gray-50 text-gray-700 text-xs font-medium font-ibm"
                      >
                        {grade.name}
                      </span>
                    ))}
