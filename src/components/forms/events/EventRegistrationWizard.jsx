...
import { setColorPreferences, setTheme, setCustomLogo } from "@store/theme/themeSlice";
...
const EventRegistrationWizard = ({ event }) => {
  ...
  const basePrice = useMemo(() => {
    ...
    return discounted && Number(discounted) < Number(price) ? Number(discounted) : Number(price);
  ...
  const tamaraImage = [{ image: locale === "ar" ? tamaraArabic : tamaraEnglish, name: "tamara" }];
    ...
    async (validateForm, setTouched, values) => {
      ...
      if (Object.keys(errors).length === 0) {
        try {
          ...
          inputs.forEach((input) => {
            const val = values[input.key];
            if (input.type === "image" || input.type === "audio" || input.type === "video") {
              if (Array.isArray(val)) {
                val.forEach((file, index) => {
                  if (file instanceof File) {
                    formData.append(`${input.key}_${index}`, file);
                    hasFiles = true;
                  }
                });
                dynamicFormInfo.push({
                  ...
                  value: Array.isArray(val)
                    ? val.map((f) => (f instanceof File ? f.name : String(f))).join(", ")
                    : "",
                });
              } else if (val instanceof File) {
                formData.append(input.key, val);
                hasFiles = true;
                dynamicFormInfo.push({ key: input.key, value: val.name });
              } else {
                dynamicFormInfo.push({ key: input.key, value: val ? String(val) : "" });
              }
              ...
              dynamicFormInfo.push({ key: input.key, value: val ? "true" : "false" });
            } else {
              dynamicFormInfo.push({
                key: input.key,
          ...
          let response;
          if (hasFiles) {
            formData.append("dynamicFormInfo", JSON.stringify(dynamicFormInfo));
            response = await axios.post(getProxyUrl(B2B_END_POINTS.EVENT_BOOKING.CLIENT_INFO), formData, {
              headers: { ...headers, "Content-Type": "multipart/form-data" },
            });
          } else {
            response = await axios.post(getProxyUrl(B2B_END_POINTS.EVENT_BOOKING.CLIENT_INFO), {
              name,
              eventTrip: event._id,
              dynamicFormInfo,
            }, { headers });
          ...
          enqueueSnackbar(error.response?.data?.message || t("eventTrips.validation.error"), { variant: "error" });
        } finally {
  ...
  const buildApiBody = (regValues, paymentValues) => {
    ...
    if (currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.CREDIT_CARD && paymentValues) {
    ...
    } else if (currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.TAMARA && paymentValues) {
      body.tamaraUserInfo = {
  ...
  const handlePaymentSubmit = async (paymentValues, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const endpoint = currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.TAMARA
          ? B2B_END_POINTS.BOOKING_EVENT_TRIP.TAMARA_INITIATION
          : B2B_END_POINTS.BOOKING_EVENT_TRIP.INITIATION;
      const response = await axios.post(getProxyUrl(endpoint), data, { headers });
  ...
  const hasDiscount = event?.discountedPrice && Number(event.discountedPrice) < Number(event.price);
  const effectiveBasePrice = hasDiscount ? Number(event.discountedPrice) : Number(event.price || 0);

  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      <WizardHero event={event} dynamicPrice={dynamicPrice} locale={locale} t={t} />

      <div className="px-6 md:px-10 py-8">
        <WizardStepper steps={STEPS} currentStep={currentStep} formTitle={formTitle} t={t} />

        {currentStep === 0 && (
          <Formik
            initialValues={registrationValuesRef.current || registrationInitialValues}
        ...
        {currentStep === 1 && (
          <Formik
            ...
            validationSchema={currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.CREDIT_CARD ? creditSchema : currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.TAMARA ? tamaraSchema : undefined}
            onSubmit={handlePaymentSubmit}
            validateOnChange={true}
            validateOnBlur={true}
          >
            {({ values: paymentValues, errors: paymentErrors, touched: paymentTouched, handleChange: handlePaymentChange, handleBlur: handlePaymentBlur, setFieldValue, isSubmitting, isValid, submitForm }) => (
              <div className="space-y-6 text-start">
                <div className="bg-activityDetailsBg rounded-xl p-5 md:p-6 border border-border">
                  <h3 className="text-base font-semibold text-titleColor font-somar mb-3">{t("eventTrips.payment.orderSummary")}</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-sm font-somar">
                      <span className="text-textLight">{event.name}</span>
                      <span className="text-textLight font-ibm">ET-{event.orderId}</span>
                    </div>
                    <hr className="border-border" />
                    <div className="flex justify-between items-center font-somar text-sm text-textLight">
                      <span>{t("eventTrips.price.basePrice") || "Base Price"}</span>
                      <div className="flex flex-col items-end gap-0.5">
                        {event.discountedPrice && Number(event.discountedPrice) < Number(event.price) ? (
                          ...
                          <span className="font-semibold font-ibm">{formatCurrency(event.price)}</span>
                        )}
                      </div>
                    </div>
                    {selectedOptionsBreakdown.map((opt, idx) => (
                      <div key={idx} className="flex justify-between items-center font-somar text-sm text-textLight pl-2 border-l-2 border-mainColor/25">
                        <span>{opt.fieldTitle}: <strong className="text-textDark font-medium">{opt.label}</strong></span>
                        <span className="font-semibold font-ibm">+ {formatCurrency(opt.price)}</span>
                      </div>
                    ))}
                    <hr className="border-border" />
                    <div className="flex justify-between items-center">
                      <span className="text-lg font-bold text-titleColor font-somar">{t("eventTrips.payment.total")}</span>
                      <div className="text-end">
                        <span className="text-lg font-bold text-mainColor font-somar flex justify-end font-ibm">{formatCurrency(dynamicPrice)}</span>
                        <p className="text-xs text-textLight font-somar">{t("eventTrips.payment.includingVat")}</p>
                      </div>
                    </div>
                  </div>
                </div>

                <RadioGroup value={currentPaymentMethod} onChange={handlePaymentMethodChange} className="flex flex-col w-full gap-4">
                  <div className="flex flex-col">
                    <PaymentMethod value={CONSTANT_VALUES.PAYMENT_METHODS.CREDIT_CARD} currentPaymentMethod={currentPaymentMethod} label={t("forms.methods.credit")} imagesList={creditImages} />
                    {currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.CREDIT_CARD && (
                      <div className="px-4 py-6 bg-[#FAF9F9] rounded-b-xl flex flex-col gap-6">
                        <TextInputGroup label={t("forms.cardholderName.name")} type="text" name="cardholderName" value={paymentValues.cardholderName} errors={paymentErrors.cardholderName} touched={paymentTouched.cardholderName} onChange={handlePaymentChange} onBlur={handlePaymentBlur} labelFontFamily="var(--font-somar-sans), sans-serif" />
                        <TextInputGroup label={t("forms.cardNumber.name")} type="text" name="cardNumber" inputMode="numeric" value={paymentValues.cardNumber} errors={paymentErrors.cardNumber} touched={paymentTouched.cardNumber} onChange={handlePaymentChange} onBlur={handlePaymentBlur} minLength="15" maxLength="16" labelFontFamily="var(--font-somar-sans), sans-serif" />
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          <div className="flex flex-col gap-2">
                            <label className="font-medium font-somar">{t("forms.expirationDate.title")}</label>
                            <div className="flex gap-3">
                              <SelectionGroupWrapper name="expiryMonth" value={paymentValues.expiryMonth} onChange={handlePaymentChange} onBlur={handlePaymentBlur} touched={paymentTouched.expiryMonth} errors={paymentErrors.expiryMonth} placeholder={t("common.allMonths")} list={months} />
                              <SelectionGroupWrapper name="expiryYear" value={paymentValues.expiryYear} onChange={handlePaymentChange} onBlur={handlePaymentBlur} touched={paymentTouched.expiryYear} errors={paymentErrors.expiryYear} placeholder={t("common.years")} list={years} />
                            </div>
                          </div>
                          <TextInputGroup label={t("forms.cvc.name")} type="text" name="cvc" inputMode="numeric" value={paymentValues.cvc} errors={paymentErrors.cvc} touched={paymentTouched.cvc} onChange={handlePaymentChange} onBlur={handlePaymentBlur} minLength="3" maxLength="4" labelFontFamily="var(--font-somar-sans), sans-serif" />
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col">
                    <PaymentMethod value={CONSTANT_VALUES.PAYMENT_METHODS.APPLE} currentPaymentMethod={currentPaymentMethod} label={t("forms.methods.applePay")} imagesList={appleImage} />
                    {currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.APPLE && (
                      <div className="px-4 py-6 bg-[#FAF9F9] rounded-b-xl">
                        <EventAppleWidget baseData={buildAppleApiBody(registrationValuesRef.current)} price={dynamicPrice} />
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col">
                    <PaymentMethod value={CONSTANT_VALUES.PAYMENT_METHODS.TAMARA} currentPaymentMethod={currentPaymentMethod} label={t("forms.methods.tamara")} imagesList={tamaraImage} />
                    {currentPaymentMethod === CONSTANT_VALUES.PAYMENT_METHODS.TAMARA && (
                      <div className="px-4 py-8 bg-[#FAF9F9] rounded-b-xl flex flex-col gap-4">
                        <TamaraWidget price={dynamicPrice} publicKey={process.env.NEXT_PUBLIC_TAMARA_WIDGET_KEY} currency="SAR" paymentType="installment" />
                        <PhoneInputWithCountrySelect countries={["SA", "AE", "BH", "KW", "OM"]} defaultCountry="SA" value={paymentValues.tamaraMobile} onChange={(val) => setFieldValue("tamaraMobile", val || "")} onBlur={handlePaymentBlur} className={cn("flex w-full bg-white p-4 border-2 rounded-lg", paymentErrors.tamaraMobile && paymentTouched.tamaraMobile ? "border-error" : "border-border")} />
                      </div>
                    )}
                  </div>
                </RadioGroup>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-8">
                  <button type="button" onClick={handleBack} className="sm:flex-1 py-3 px-6 border-2 border-gray-300 rounded-xl font-semibold hover:bg-gray-50">{t("pagination.previous")}</button>
                  <button type="button" disabled={isSubmitting || isPaymentSubmitting || !isValid} onClick={submitForm} className="sm:flex-1 py-3.5 px-6 bg-mainColor text-white rounded-xl font-semibold text-lg flex items-center justify-center gap-2">
                    {isSubmitting || isPaymentSubmitting ? <CircularProgress size={20} color="inherit" /> : t("eventTrips.payment.confirm")}
                  </button>
                </div>
              </div>
            )}
...
const SelectionGroupWrapper = ({ name, value, onChange, onBlur, touched, errors, placeholder, list }) => (
  <div className="flex-1">
    <Select name={name} value={value} onChange={onChange} onBlur={onBlur} displayEmpty sx={{ width: "100%", borderRadius: "8px" }}>
      <MenuItem value="" disabled>{placeholder}</MenuItem>
      {list.map((item) => (
        <MenuItem key={item} value={name === "expiryYear" ? item.toString().slice(-2) : item}>{item}</MenuItem>
      ))}
    </Select>
  </div>
);
