...
/**
 * Reusable bilingual (en + ar) list step.
 * Used by StepMustHave, StepExemptions, StepBenefits.
 *
 * @param {string} fieldName        - Formik field path (e.g. "mustHaveItems")
 * @param {string} titleKey         - t() key for the section title
 * @param {string} subtitleKey      - t() key for the section subtitle
 * @param {string} itemLabelKey     - t() key for the item row label (receives {num})
 * @param {string} enLabelKey       - t() key for the EN input label
 * @param {string} arLabelKey       - t() key for the AR input label
 * @param {string} enPlaceholderKey - t() key for the EN input placeholder
 * @param {string} arPlaceholderKey - t() key for the AR input placeholder
 * @param {string} addButtonKey     - t() key for the add button label
 */
const BilingualListStep = ({
  fieldName,
  titleKey,
  subtitleKey,
  itemLabelKey,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h4 className="text-sm font-bold text-titleColor">{t(titleKey)}</h4>
        <p className="text-xs text-subtitleColor">{t(subtitleKey)}</p>
      </div>

      <FieldArray name={`${fieldName}.en`}>
...
export default BilingualListStep;
