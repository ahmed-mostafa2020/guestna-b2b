    ...
    >
      <div className="flex justify-between flex-1 sm:hidden">
        <button
          ...
          {...getGtmTag(GTM_TAGS.PAGINATION.PREV, "pagination")}
        >
        ...
        <button
          ...
          {...getGtmTag(GTM_TAGS.PAGINATION.NEXT, "pagination")}
        >
          {t("pagination.next")}
        </button>
      </div>
        ...
        <div>
          <p className="text-sm text-gray-700">
            ...
            <span className="font-medium">
              {pageInfo.total || pageInfo.totalCount || 0}
            </span>{" "}
            {" , "}
            {t("pagination.show")} {t("pagination.of")}{" "}
            <span className="font-medium">
              {(activePage - 1) * (pageInfo.perPage || 10) + 1}
            </span>{" "}
            {t("pagination.to")}{" "}
            <span className="font-medium">
              {Math.min(
                activePage * (pageInfo.perPage || 10),
                pageInfo.total || pageInfo.totalCount || 0
              )}
            </span>
          </p>
